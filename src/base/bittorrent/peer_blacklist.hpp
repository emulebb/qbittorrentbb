...
// qBittorrentBB additions: a configurable auto-ban policy, populated by
// SessionImpl before the extensions are registered. It generalizes Enhanced's
// hardcoded China-only rules into a configurable country list, and adds a
// whole-country block (ban every peer from a country, regardless of client).
struct AutoBanPolicy
{
    QStringList blockedCountries;    // ban ALL peers from these countries (any client)
    QStringList unknownCountries;    // ban "unknown" clients only from these countries
    bool unknownEverywhere = false;  // ban "unknown" clients regardless of country
};

inline AutoBanPolicy g_autoBanPolicy;

inline QString peer_country(const lt::peer_info &info)
{
    return Net::GeoIPManager::instance()->lookup(QHostAddress(info.ip.data()));
}
...
// Unknown Peer filter (qBittorrentBB: configurable countries / everywhere,
// generalizing Enhanced's China-only rule).
bool is_unknown_peer(const lt::peer_info& info)
{
  if (info.client.find("Unknown") == std::string::npos)
    return false;
  if (g_autoBanPolicy.unknownEverywhere)
    return true;
  if (g_autoBanPolicy.unknownCountries.isEmpty())
    return false;
  return g_autoBanPolicy.unknownCountries.contains(peer_country(info));
}

// Whole-country block (qBittorrentBB addition): ban every peer from a configured
// country, regardless of client. Empty list => disabled.
bool is_blocked_country(const lt::peer_info& info)
{
  if (g_autoBanPolicy.blockedCountries.isEmpty())
    return false;
  return g_autoBanPolicy.blockedCountries.contains(peer_country(info));
}
    ...
    action_function action)
{
  ...
  // qBittorrentBB: never filter the DHT harvester's ephemeral metadata fetches
  // (added in upload_mode). The harvester is exempt from all ban rules -- it must
  // keep every peer for discovery; bans protect upload on real torrents only.
  if (th.flags() & lt::torrent_flags::upload_mode)
    return nullptr;
